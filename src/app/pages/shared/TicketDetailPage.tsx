...
const ROLE_CHAT_COLOR: Record<string, string> = {
  CONSULTANT_TECHNIQUE: 'border-blue-400',
  CONSULTANT_FONCTIONNEL: 'border-green-400',
  MANAGER: 'border-purple-400',
  COORDINATEUR_DEV: 'border-purple-400',
  CHEF_DE_PROJET: 'border-gray-400',
  ADMIN: 'border-gray-400',
};
...
const ROLE_LABEL: Record<string, string> = {
  CONSULTANT_TECHNIQUE: 'Tech',
  CONSULTANT_FONCTIONNEL: 'Func',
  MANAGER: 'Manager',
  COORDINATEUR_DEV: 'Coord',
  CHEF_DE_PROJET: 'Chef PJ',
  ADMIN: 'Admin',
};
...
export const TicketDetailPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Hydrate messages and activity from ticket
      setMessages(found.messages ?? []);
      setActivityFeed(found.activityFeed ?? []);
    } finally {
      setLoading(false);
    }
  };
  ...
  const addActivity = useCallback(
    (
      ...
      setActivityFeed((prev) => [evt, ...prev]);
      // Persist to ticket
      const updatedFeed = [evt, ...(ticket.activityFeed ?? [])];
      void TicketsAPI.update(ticket.id, { activityFeed: updatedFeed });
    },
    [currentUser, ticket],
  );
  ...
  const sendMessage = () => {
    if (!currentUser || !ticket || !msgInput.trim()) return;
    const msg: Message = {
    ...
    setMsgInput('');
    // Persist
    void TicketsAPI.update(ticket.id, { messages: updatedMessages });
    addActivity(
      'message_sent',
      `${currentUser.name} a envoyé un message`,
    );
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Three-column layout */}
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-[2fr_1.5fr_1fr] gap-0 overflow-hidden">
        {/* ================================================================
        ...
        {/* ================================================================
            CENTER COLUMN — Messagerie
        ================================================================ */}
        <div className="flex flex-col h-[calc(100vh-8rem)] border-r">
            ...
            <div className="p-4 space-y-3">
              ...
              {messages.map((msg) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${ROLE_AVATAR_BG[msg.senderRole] ?? 'bg-muted'}`}
                    >
                    ...
                    <div className={`max-w-[80%] ${isMe ? 'text-right' : ''}`}>
                      ...
                      <div
                        className={`rounded-lg px-3 py-2 text-sm border-l-4 ${
                          ROLE_CHAT_COLOR[msg.senderRole] ?? 'border-gray-300'
                        } ${
                          isMe
