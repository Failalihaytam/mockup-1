...
import { ProjectsAPI, TicketsAPI, WorkSessionsAPI, UsersAPI } from '../../services/odataClient';
import { Project, Ticket, WorkSession, User } from '../../types/entities';
...
export const ImputationsEquipe: React.FC = () => {
  ...
  const consultants = useMemo(() => {
    const ids = [...new Set(sessions.map((tl) => tl.consultantId))];
  ...
  }, [sessions, consultantFilter, projectFilter, dateFrom, dateTo]);
  ...
  const consultantSummary = useMemo(() => {
    ...
    sessions.forEach((tl) => {
      ...
      map[tl.consultantId].total += tl.hours;
      if (tl.sentToStraTIME) map[tl.consultantId].sent += tl.hours;
      else map[tl.consultantId].pending += tl.hours;
    });
  ...
  const projectSummary = useMemo(() => {
    ...
    filteredLogs.forEach((tl) => {
      ...
      map[tl.projectId].total += tl.hours;
      if (tl.sentToStraTIME) map[tl.projectId].sent += tl.hours;
    });
  ...
  const pendingTotal = useMemo(
    () => sessions.filter((tl) => !tl.sentToStraTIME).reduce((s, tl) => s + tl.hours, 0),
    [sessions],
  );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <p className="text-2xl font-bold">
              {formatHours(sessions.reduce((s, tl) => s + tl.hours, 0))}
            </p>
          </div>
          <div className="rounded-lg border bg-card p-4">
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
            ...
            <TableBody>
              {consultantSummary.map((c) => (
                <TableRow key={c.id}>
                  ...
                  <TableCell className="px-4 py-3 font-mono text-sm">{formatHours(c.total)}</TableCell>
                  <TableCell className="px-4 py-3 font-mono text-sm text-emerald-600 dark:text-emerald-400">{formatHours(c.sent)}</TableCell>
                  <TableCell className="px-4 py-3 font-mono text-sm">
                    {c.pending > 0 ? (
                      <span className="text-amber-600 dark:text-amber-400">{formatHours(c.pending)}</span>
                    ) : (
              ...
              {projectSummary.map((p) => {
                ...
                return (
                  <div key={p.id} className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-muted-foreground w-16 text-right">{formatHours(p.total)}</span>
            ...
            <div className="rounded-lg border bg-card overflow-x-auto">
              ...
              {filteredLogs.length > 0 && (
                <div className="border-t px-4 py-2 text-sm text-muted-foreground flex justify-between">
                  ...
                  <span className="font-medium">Total: {formatHours(totalFiltered)}</span>
                </div>
              )}
            </div>
          )}
